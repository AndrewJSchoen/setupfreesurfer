...
${current}/palantir cell ${MONITOR_DIR} -r Project -c Extract --settext "Running" --setanimate "bars" --setbgcolor "#efd252" --settxtcolor "#ec6527" --addnote "Started running"

if [ $HOSTNAME != $DESIRED_HOSTNAME ] ; then
  echo "ERROR: NOT ON CORRECT HOST FOR RUNNING FREESURFER"
  echo "ABORTING PROCESS"
  ${current}/palantir cell ${MONITOR_DIR} -r Project -c Extract --settext "Host Error" --setanimate "toggle" --setbgcolor "#cb3448" --settxtcolor "#791f2b"
  exit 1
fi
errorcode=0
subjects=""
...
if [[ $errorcode == 0 ]] ; then
  ${current}/palantir cell ${MONITOR_DIR} -r Project -c Extract --settext "Finished" --setanimate "none" --setbgcolor "#009933" --settxtcolor "#004c19" --addnote "Successfully finished"
else
  ${current}/palantir cell ${MONITOR_DIR} -r Project -c Extract --settext "Error" --setanimate "toggle" --setbgcolor "#cb3448" --settxtcolor "#791f2b" --addnote "Error"
  exit 1
fi

...
if [[ $phase == "base" ]] ; then
  for timepoint in $timepoints ; do
    ${current}/palantir cell ${MONITOR_DIR} -r ${subject_id}_${timepoint} -c View --settext "Running" --setanimate "bars" --setbgcolor "#efd252" --settxtcolor "#ec6527" --addnote "Opened ${view_type} viewing for ${phase}"
  done
else
  ${current}/palantir cell ${MONITOR_DIR} -r ${monitor_id} -c View --settext "Running" --setanimate "bars" --setbgcolor "#efd252" --settxtcolor "#ec6527" --addnote "Opened ${view_type} viewing for ${phase}"
fi

eval commandstring

${current}/palantir cell ${MONITOR_DIR} -r ${fs_id} -c View --settext "Finished" --setanimate "none" --setbgcolor "#009933" --settxtcolor "#004c19" --addnote "Closed"

if [[ $phase == "base" ]] ; then
  for timepoint in $timepoints ; do
    ${current}/palantir cell ${MONITOR_DIR} -r ${subject_id}_${timepoint} -c View --settext "Finished" --setanimate "none" --setbgcolor "#009933" --settxtcolor "#004c19" --addnote "Closed"
  done
else
  ${current}/palantir cell ${MONITOR_DIR} -r ${monitor_id} -c View --settext "Finished" --setanimate "none" --setbgcolor "#009933" --settxtcolor "#004c19" --addnote "Closed"
fi
